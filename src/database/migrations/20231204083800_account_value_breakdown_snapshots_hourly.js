...
exports.up = (knex) => knex.schema.createTable(ACCOUNT_VALUE_BREAKDOWN_SNAPSHOT_HOURLY_TABLE, table => {
    table.increments();
  ...
  .then(async (snapshots) => {
    for(let snapshot of snapshots) {
      if(((new Date(snapshot.timestamp).getTime() / 1000) % (60*60) === 0)) {
        await knex(ACCOUNT_VALUE_BREAKDOWN_SNAPSHOT_HOURLY_TABLE).insert({
...
exports.down = knex => knex.schema.dropTable(ACCOUNT_VALUE_BREAKDOWN_SNAPSHOT_HOURLY_TABLE);