...
class BalanceController extends Controller {
  ...
  async createERC20BalanceShim(req: Request, res: Response) {
    ...
    // Check if a balance shim for this token already exists for this account
    let existingBalanceShimRecord = await BalanceShimRepository.getBalanceShimByAssetAndHolder(token_address, holder_address);

    if(!existingBalanceShimRecord) {
      // Create new balance shim record
      await BalanceShimRepository.create({
      ...
      // Update balance shim record
      await BalanceShimRepository.update({
        balance: balance,
      }, existingBalanceShimRecord.id);
    }
